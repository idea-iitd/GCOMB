...
for num_k in [150]:# [10,20,50,100,150,200]:
	...
	for iter in range(0, num_of_iterations_imm):
		...
		out_file_path = "../../../../../" + graph_path + "large_graph_ic_imm_sol_eps" + str(epsilon) + "_num_k_" + str(
			num_k) +"_iter_"+str(iter)

		print(out_file_path)
		result_time_file_name = "../../../../../" + graph_path + "large_graph_ic_imm_time_eps" + str(
		...
		command = "time ./imm_discrete -dataset " + dataset_path + " -k " + str(
			num_k) + " -model IC -epsilon " + str(epsilon)+" -output " + out_file_path + " -seed_random " + str(seed_random)+ " -training_for_gain 0"
	...
	#
	# 	pickle.dump(dict_togetherness, handle, protocol=pickle.HIGHEST_PROTOCOL)
