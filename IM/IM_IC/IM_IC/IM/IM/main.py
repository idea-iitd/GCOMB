    ...
    #	previous_spread= shortReward
        ...
        if (step > (windowSize)):
            ...
            if(len(historyOfTuples)>5) :
                ...
                graph_path = "./GraphSAGE-master/real_data/youtube/IC/train/large_graph"
                command="python get_output.py " + graph_path +" "+ str(50) +" 0.05 None > /dev/null"
                print(command)
                os.system(command)

                rl_result_file = graph_path+'-result_RL_50_nbs_0.05'
                ...
                print("seed nodes ", int_selected_nodes)
                seed_nodes=[str(i) for i in int_selected_nodes]
                print("joined ", "\n".join(seed_nodes))
                file_temp_seed_select.write("\n".join(seed_nodes))

                file_temp_seed_select.close()
                os.chdir("Executables/")
                os.system("python run_eval.py " + "../" + temp_seed_file_name + " " + str(
                    len(seed_nodes)) + " " + "../temp_reward.txt  > /dev/null")
