...
def predict(train_data, test_data=None):
	...
	minibatch = NodeMinibatchIterator(G,
			...
			context_pairs = context_pairs,
									  budget=num_k,bud_mul_fac=FLAGS.bud_mul_fac,mode="test", prefix = FLAGS.train_prefix, sampling_freq=FLAGS.neighborhood_sampling
									  )
...
#	val_adj_info = tf.assign(adj_info, minibatch.test_adj)
	...
	saver.restore(sess,"ICYoutubesupervisedTrainedModel_MC_marginal/model.ckpt")
...
#	bottom_nodes, top_nodes = bipartite.sets(G)
	...
	result_top_percent = FLAGS.train_prefix + "_top_ten_percent_analyse.txt"+"_"+str(num_k)+"_nbs_"+ str(FLAGS.neighborhood_sampling)
...
#		if sorted_dict[y][0] in bottom_nodes:
	...
	result_file_name = FLAGS.train_prefix + "_sup_GS_sol.txt"+"_"+str(num_k)+"_nbs_"+str(FLAGS.neighborhood_sampling)
...
#	file_handle.write("\n")
   ...
   # np.save(embedding_file_name,embeddings)
	...
	time_rl_prep_file_name=FLAGS.train_prefix + "_num_k_" + str(FLAGS.num_k) + "_time.txt"+"_"+str(num_k)+"_nbs_"+str(FLAGS.neighborhood_sampling)
