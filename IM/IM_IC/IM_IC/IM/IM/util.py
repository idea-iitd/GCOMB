...
class Graph:
	def __init__(self, dimEmbedding, episodeNum, num_k):
		cons_to_add = 5000
		self.graph_dir = "./GraphSAGE-master/real_data/youtube/IC/train/large_graph"
		...
		(self.graphX, unscaled_embedding, self.numNodes, self.top_tenpct_nodes,self.dict_node_sampled_neighbors) = getNewGraph(self.graph_dir, num_k, 0.05)
...
def getShortReward(nodeSelected, graphid, previous_spread):
	...
	if(len(graphEnv.graphEnvironment[graphid].state)==0):
		print(" first step ")
		shortReward = calculate_spread_mc_sim(graphEnv.graphEnvironment[graphid].graph_dir, [nodeSelected],"IC")
...
#	print("before ",initial_spread, seed_nodes_earlier, new_node_to_be_added)
	...
	final_spread  = calculate_spread_mc_sim(graphEnv.graphEnvironment[graphid].graph_dir, seed_nodes_later,"IC")
	...
	shortReward =max(0, final_spread - previous_spread)
