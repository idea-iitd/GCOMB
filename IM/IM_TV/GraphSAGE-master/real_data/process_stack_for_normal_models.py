...
dataset="stack"
input_file_path = dataset+"/stack.txt"
...
while True:
    ...
    file_write.write( str(node_dic[source]) + ' ' + str(node_dic[target] )+' ' + str(inf_prob)+'\n')
    #id_map[str(node_dic[source])] = node_dic[source]
    #id_map[str(node_dic[target])] = node_dic[target]

    UG.add_edge(node_dic[source], node_dic[target], weight=inf_prob)
