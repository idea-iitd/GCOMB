...
int model=1; /*********** 0 for IC, 1 for WC, 2 for LT ***************/
...
double calculate_flow(Graph& h, vector<unsigned   int>& seed, double lambda ){
	int i=0;
	...
	for(;i<tempSeed.size();i++){
		if(g[tempSeed[i]].first.covered ==false){		
			...
			for(int it=0;it<neigh_list.size();it++){
				if(model==1){
					thresh=1.0/(float)g[neigh_list[it]].first.in_degree;
				}
				if((double)rand()/RAND_MAX <= thresh && g[neigh_list[it]].first.covered==false){//added_now
				//if(g[it->first].first.covered==0){
...
/*************************************************************************************/
double calculate_flow_update(Graph& g, vector<unsigned   int>& seed, double lambda ){
	int i=0;
	unordered_map<unsigned   int,bool> spread;
	vector<unsigned   int> tempSeed=seed, neigh_list;
	float thresh;
...
int main(int argc,char *argv[]){
    ...
    for(unsigned int seed:numSeeds){
	    ...
	    for(int i=0;i<num_iter;i++){
		    ...
		    clock_t start = clock();
		    running_spread=calculate_flow(g,flowSeeds,lambda);
		    clock_t end = clock();
