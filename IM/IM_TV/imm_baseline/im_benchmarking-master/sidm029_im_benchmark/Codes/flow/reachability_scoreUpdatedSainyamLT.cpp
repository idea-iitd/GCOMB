...
int model=2; /*********** 0 for IC, 1 for WC, 2 for LT ***************/
...
double lt_flow_efficient(Graph& h, vector<unsigned int>& seed, double lambda){
	int i=0;
	unordered_map<unsigned int,bool> spread;
	
	//Graph g_active(g.size());
	Graph g=h;
	vector<unsigned int> tempSeed=seed, out_neigh_list;

	//vector<bool> visited(g.size(),false);
	/*
	for(int nodeId=0;nodeId<g.size();nodeId++){
		g[nodeId].first.thresh=((float)rand()/RAND_MAX);
	}
	*/
	for(;i<tempSeed.size();i++){
		if(g[tempSeed[i]].first.covered==false){
			out_neigh_list = g[tempSeed[i]].second;
			//it = out_neigh_list.begin();
			//while(it!=out_neigh_list.end()){
			for(int it=0;it<out_neigh_list.size();it++){
				/*
				if(visited[out_neigh_list[it]]==false){
					g[out_neigh_list[it]].first.thresh=((float)rand()/RAND_MAX);
					visited[out_neigh_list[it]]=true;
				}
				*/
				g[out_neigh_list[it]].first.thresh-=1.0/(float)g[out_neigh_list[it]].first.in_degree;
				if(g[out_neigh_list[it]].first.thresh<=0 && g[out_neigh_list[it]].first.covered==false){//added_now
					tempSeed.push_back(out_neigh_list[it]);
					if(spread.find(out_neigh_list[it])==spread.end()){
						spread[out_neigh_list[it]]=true;
					}
				}
			}
		}
...
/********************* This function works correctly only for undirected Graphs (graphs that are undirected in nature) ****************************/
double lt_flow(Graph& h, vector<unsigned int>& seed, double lambda){
	int i=0;
    Graph g = h;
	unordered_map<unsigned int,bool> spread;
	...
	for(int nodeId=0;nodeId<g.size();nodeId++){
		vector<unsigned int> active_adj;
		active_adj.clear();

		out_neigh_list=g[nodeId].second;
		//it=out_neigh_list.begin();
		g_active[nodeId].first=g[nodeId].first; // Create the destination node
		
		unsigned int edge_advance=((float)rand()/RAND_MAX)*g[nodeId].first.in_degree; // Find the edge position according to the interval of the live_edge_model
		if(edge_advance==g[nodeId].first.in_degree){ // To save the corner-case when in_degree == edge_advance or when rand()/RAND_MAX ==1
			edge_advance-=1;
		}
		//it=next(it,edge_advance); // Advance to that edge
		//g_active[it->first].first=g[it->first].first; // Now create the source node; Eventually this node will be added
		//if(g_active[it->first].second.size()==0){
			//active_adj[nodeId]=true;
		if(g_active[out_neigh_list[edge_advance]].second.size()==0){
			active_adj.push_back(nodeId);
			g_active[out_neigh_list[edge_advance]].second=active_adj; // The adjacency list of node v, would contain node u
		}
		else{
			//g_active[it->first].second[nodeId]=true; // The adjacency list of node v, would contain node u
			g_active[out_neigh_list[edge_advance]].second.push_back(nodeId); // The adjacency list of node v, would contain node u
		}
	}
	for(;i<tempSeed.size();i++){
		if(g[tempSeed[i]].first.covered==false){
			out_neigh_list = g_active[tempSeed[i]].second;
			//it = out_neigh_list.begin();
			//while(it!=out_neigh_list.end()){
			for(int it=0;it<out_neigh_list.size();it++){
				tempSeed.push_back(out_neigh_list[it]);
...
double lt_flow_update(Graph& g, vector<unsigned int>& seed, double lambda){
	int i=0;
	unordered_map<unsigned int,bool> spread;
	...
	for(int nodeId=0;nodeId<g.size();nodeId++){
		...
		//it=out_neigh_list.begin();
		g_active[nodeId].first=g[nodeId].first; // Create the destination node
		g_active[nodeId].first.covered=false; // Initializing covered to be false for the BFS in the following code
	...
	//cout<<"Starting BFS on g_active"<<endl;
	for(;i<tempSeed.size();i++){
		if(g_active[tempSeed[i]].first.covered==false){
			out_neigh_list = g_active[tempSeed[i]].second;
			//it = out_neigh_list.begin();
			//while(it!=out_neigh_list.end()){
			for(int it=0;it<out_neigh_list.size();it++){
				tempSeed.push_back(out_neigh_list[it]);
				if(g[out_neigh_list[it]].first.covered==false && spread.find(out_neigh_list[it])==spread.end()){
					spread[out_neigh_list[it]]=true;
				}
			}
		}
		g_active[tempSeed[i]].first.covered=true;
		g[tempSeed[i]].first.covered=true;
	}
	double spread_val=(double)spread.size();
	vector<unsigned int>().swap(tempSeed);
...
/********************* This function works correctly only for undirected Graphs ****************************/

double kempe(Graph& g, int numSeeds, double lambda, vector<unsigned   int>& seed, int num_iter){
	int i=0;
	unsigned   int maxId;
		...
		for(unsigned   int nodeId=0;nodeId<g.size();nodeId++){
			...
			if(model==2){
                for(int iter=0;iter<num_iter;iter++){
                    new_flow+=lt_flow(g,seed,lambda);
                }
		...
		for(unsigned   int nodeId=0; nodeId<g.size(); nodeId++){
			if(g[nodeId].first.covered==false)
			{
				...
				for(int it=0;it<neigh_list.size();it++){
					if(model==2){//LT
						prob=1.0/(float)g[neigh_list[it]].first.in_degree;
					}
...
double select_reachability_path_seed (Graph &h, int k, int num_seeds, vector<unsigned   int>& seed,double lambda)
{
	int i=0;
	...
	for(;i<num_seeds;i++)
	{
		...
		if(model==2){	
			curr_flow = lt_flow_update(g, tmpSeed, lambda);
		}
		seed.push_back(maxId);
...
int main(int argc,char *argv[]){
    ...
    //detailedOpFile.open(argv[8],ios::out);
    ...
    if(inpFile.is_open()){
        while(inpFile>>vert1>>vert2>>prob){
            ...
            if(id2index.find(vert1)==id2index.end()){
                ...
                up.first = u;
                up.second = adj_list;
				up.first.in_degree=0;//added_now
                g.push_back(up);
                indexu = index;
                index++;
            }
            ...
            if(id2index.find(vert2)==id2index.end()){
                ...
                vp.first = v;
				vp.first.in_degree=0;//added_now
                vp.second = adj_list;
                g.push_back(vp);
                indexv = index;
                index++;
            ...
            if(gType==0){ // Undirected
                ...
                if(indexu!=indexv){
					...
					g[indexu].first.in_degree++;//added_now
					g[indexv].first.in_degree++;//added_now
                }
    ...
    float flowTime;
	//statOpFile<<"#Seeds\tSpread\tTime"<<endl;
	...
	spreadOpFile.open(spreadFileName+"_"+to_string((long long int)seed),ios::out | ios_base::app);
	for(int i=0;i<num_iter;i++){
		for(int nodeId=0;nodeId<g.size();nodeId++){
			g[nodeId].first.thresh=((float)rand()/RAND_MAX);
		}
		clock_t start = clock();
		running_spread=lt_flow_efficient(g,flowSeeds,lambda);
		clock_t end = clock();
                float seconds = (float)(end - start) / CLOCKS_PER_SEC;
		flowTime+=seconds;
		spreadVec.push_back(running_spread+seed);
		spreadOpFile<<running_spread+seed<<endl;
	}
	...
	for(int bin=0;bin<binSize.size();bin++){
		...
		statOpFile<<seed<<"\t"<<spread<<"\t"<<stdSpread<<"\t"<<flowTime<<endl;
		meanSpread.clear();
		statOpFile.close();
	}
	spreadOpFile.close();
	...
	if(argc!=9){
		cout<<"Usage:"<<endl;
		cout<<argv[0]<<" <path-to-graph-file> <choice-for-function> <#num-seeds> <lambda> <#num-iterations> <#depth-for-reachability&path> <path-to-stat-file> <path-to-detailedFile>"<<endl;
		cout<<"Choice 0 -- Kempe"<<endl;
		cout<<"Choice 1 -- Reachability"<<endl;
		cout<<"Choice 2 -- Path"<<endl;
	...
	double decay_prob=0.1;

	inpFile.open(argv[1],ios::in);
	statOpFile.open(argv[7],ios::out);
	detailedOpFile.open(argv[8],ios::out);
	unsigned   int vert1,vert2,index=0,indexu,indexv;
	unordered_map<unsigned   int, unsigned   int> id2index;
	vector<unsigned   int> adj_list;
...
//			v.contents.clear(); 
	...
	vector<unsigned   int> seeds;
	vector<double> decayProb;
	double seed_time=0.0, sqdSeedTime=0.0, maxSeedTime=numeric_limits<double>::min(), minSeedTime=numeric_limits<double>::max(), meanSeedTime=0.0, stdSeedTime=0.0, flow_time=0.0, spread=0.0, sqdSpread=0.0, maxSpread=numeric_limits<double>::min(), minSpread=numeric_limits<double>::max(), cur_time=0.0, cur_spread=0.0, meanSpread=0.0, stdSpread=0.0;
     */
	if(ch==0){
		...
		iter=num_iter;
		/*
		   decayProb.push_back(1.0);
		   for(int i=1;i<depth;i++){
		   decayProb.push_back(pow(decay_prob,(double)i));
		   }
		 */
		while(iter>0){
#ifdef PRINTS
