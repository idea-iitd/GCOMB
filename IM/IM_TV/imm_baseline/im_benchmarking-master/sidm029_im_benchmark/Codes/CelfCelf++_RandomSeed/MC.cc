...
void MC::doAll() {
	...
	int phase = 10;//strToInt(opt->getValue("phase"));
...
//	int budget = 50;
	...
	if (pTop == NULL) {
		...
		writeInFile(pTop->nodeID, totalCov, totalCov, 0, 0, 0, countUsers, avgCountUsers);
	...
	while (curSeedSet.size() < budget && totalCov < totalUserNum) {			
		...
		if ( pCurr->flag == (curSeedSet.size() + 1) ) {	
			...
			avgCountUsers+=countUsers;
			writeInFile(pCurr->nodeID, totalCov, pCurr->gain, 0, 0, 0, countUsers, avgCountUsers);
			...
			// pick it as seed	
				...
				avgCountUsers+=countUsers;
				writeInFile(pCurr->nodeID, totalCov, pCurr->gain_next, 0, 0, 0, countUsers, avgCountUsers);	
...
float MC::mineSeedSet(int t_ub) {
	...
	//avgCountUsers=countUsers;
	writeInFile(*curSeedSet.begin(), totalCov, totalCov, t_ub, actualCov, actualCov, countUsers, avgCountUsers);
	cout << "Number of nodes examined in this iteration: " << countUsers << "\t" << avgCountUsers << endl;
...
//	while (totalCov < eta) {
	while (curSeedSet.size() < budget) {
		...
		if (flag || newCov - totalCov >= oldCov_nextElement) {
			...
			avgCountUsers+=usersExamined.size();
			writeInFile(v, newCov, newCov - totalCov, t_ub, actualCov, actualCov - prevActualCov, countUsers, avgCountUsers);
			cout << "Number of nodes examined in this iteration: " << usersExamined.size() << "\t" << avgCountUsers << endl;
//			cout << "True Cov " << trueCov << endl;
...
void MC::writeInFile(UID v, float cov, float marginal_gain, int curTimeStep, float actualCov, float actualMG, int countUsers, float avgCountUsers) {
	...
	if(size==1 || size==2 || size==5||size==10||(size%25==0&& size>10)){
		statFile<<size<<" "<<cov<<" "<<getTime()<<" "<<getCurrentMemoryUsage()<<" "<<avgCountUsers/(float)size<<endl;
	}
...
void MC::openOutputFiles() {
	...
	string filename, statfilename, seedfilename;
	
	int celfPlus = strToInt(opt->getValue("celfPlus"));	
	if (celfPlus == 1) {
		filename = outdir + "/" + "output_CELF++_" + m + "_" + to_string(countIterations) + ".txt";
		// Added by Akhil and Sainyam
		statfilename = outdir + "/" + "stats_CELF++_" + m + "_" + to_string(countIterations) + ".txt";
		seedfilename = outdir + "/" + "seeds_CELF++_" + m + "_" + to_string(countIterations) + ".txt";
		//
	} else {
		filename = outdir + "/" + "output_CELF_" + m + "_" + to_string(countIterations) + ".txt";
		// Added by Akhil and Sainyam
		statfilename = outdir + "/" + "stats_CELF_" + m + "_" + to_string(countIterations) + ".txt";
		seedfilename = outdir + "/" + "seeds_CELF_" + m + "_" + to_string(countIterations) + ".txt";
		//
	}
