...
outputcelf="../Results/freshRuns/algorithmOutput/fig6e-7e-8e/celf"
outputcelfpp="../Results/freshRuns/algorithmOutput/fig6e-7e-8e/celfpp"
outputeasyim="../Results/freshRuns/algorithmOutput/fig6e-7e-8e/easyim"
outputimm="../Results/freshRuns/algorithmOutput/fig6e-7e-8e/imm"
outputtim="../Results/freshRuns/algorithmOutput/fig6e-7e-8e/tim"
outputpmc="../Results/freshRuns/algorithmOutput/fig6e-7e-8e/pmc"
outputimrank="../Results/freshRuns/algorithmOutput/fig6e-7e-8e/imrank"
outputsg="../Results/freshRuns/algorithmOutput/fig6e-7e-8e/sg"
outputirie="../Results/freshRuns/algorithmOutput/fig6e-7e-8e/irie"
#outputsimpath="../Results/freshRuns/algorithmOutput/fig6e-7e-8e/simpath"
#outputldag="../Results/freshRuns/algorithmOutput/fig6e-7e-8e/ldag"
mkdir -p ../Results/freshRuns/processed/fig6e-7e-8e
mkdir -p ../Results/freshRuns/algorithmOutput/fig6e-7e-8e
mkdir -p $outputcelf
...
do
	...
	do
		...
		cat $outputimrank"/statIMRank_k"$seeds"_l"$l"_LOOP10_irDegree.txt" >> ../Results/freshRuns/processed/fig6e-7e-8e/imrank$l"time.txt"
	...
	cp $outputimrank"/flowstat_l="$l"_rounds="$imrank_rounds".txt_300" ../Results/freshRuns/processed/fig6e-7e-8e/imrank$l".txt"
done
cd ../Runningscript
...
do
	...
	cat $outputirie"/statFileseeds-"$seeds".txt" >> ../Results/freshRuns/processed/fig6e-7e-8e/irietime.txt
...
cp $outputirie"/flowstat_alpha=0.7_300" ../Results/freshRuns/processed/fig6e-7e-8e/irie.txt
cd ../Runningscript
...
cp $outputpmc"/stats_PMC_IC_"$pmc_r".txt" ../Results/freshRuns/processed/fig6e-7e-8e/pmctime.txt
...
cp $outputpmc"/flowstat_"$pmc_r".txt_300" ../Results/freshRuns/processed/fig6e-7e-8e/pmc.txt
...
do
	...
	cat $outputsg"/statStaticGreedyUD_SGD_R"$sg_r"_k"$seeds".txt" >> ../Results/freshRuns/processed/fig6e-7e-8e/sgtime.txt
...
cp $outputsg"/flowstat_snapshots="$sg_r"_300"  ../Results/freshRuns/processed/fig6e-7e-8e/sg.txt
...
for seeds in 1 2 5 10 25 50 75 100 125 150 175 200
do
	echo $execfileimm" -dataset "$pathtograph" -k "$seeds" -model WC -epsilon "$imm_eps" -output "$outputimm
	$execfileimm -dataset $pathtograph -k $seeds -model $model -epsilon $imm_eps -output $outputimm
	cat $outputimm"/stat_IMM_"$model"_"$seeds"_"$imm_eps".txt"  >> ../Results/freshRuns/processed/fig6e-7e-8e/immtime.txt
...
cp $outputimm"/flowstat_"$imm_eps"_300" ../Results/freshRuns/processed/fig6e-7e-8e/imm.txt
echo "Running TIM+"
for seeds in 1 2 5 10 25 50 75 100 125 150 175 200
do
	echo $execfiletim" -dataset "$pathtograph" -k "$seeds" -model WC -epsilon "$tim_eps" -outdir "$outputtim
	$execfiletim -dataset $pathtograph -k $seeds -model $model -epsilon $tim_eps -outdir $outputtim
	cat $outputtim"/stat_TIM_"$model"_"$seeds"_"$tim_eps".txt" >> ../Results/freshRuns/processed/fig6e-7e-8e/timtime.txt
...
cp $outputtim"/flowstat_"$tim_eps"_300" ../Results/freshRuns/processed/fig6e-7e-8e/tim.txt
...
do
	...
	cat $outputeasyim"/stat_seeds="$seeds"_iter="$easyim_iter"_depth=3.txt" >> ../Results/freshRuns/processed/fig6e-7e-8e/easyimtime.txt
...
cp $outputeasyim"/flowstat_"$easyim_iter".txt_300" ../Results/freshRuns/processed/fig6e-7e-8e/easyim.txt
...
echo "Running CELF" 
echo $execfilecelf" -propModel WC -mcruns "$celf_mc" -budget 200 -celfPlus 0 -probGraphFile "$pathtograph" -outdir "$outputcelf
$execfilecelf -propModel $model -mcruns $celf_mc -budget 200 -celfPlus 0 -probGraphFile $pathtograph -outdir $outputcelf
cp $outputcelf"/stats_CELF_IC_"$celf_mc".txt" ../Results/freshRuns/processed/fig6e-7e-8e/celftime.txt
echo $execFlow $pathtograph $outputcelf"/seeds_CELF_IC_"$celf_mc".txt" 200 1 $flowiter 3 $outputcelf"/flowstat_"$celf_mc".txt" $outputcelf"/spread.txt"
$execFlow $pathtograph $outputcelf"/seeds_CELF_IC_"$celf_mc".txt" 200 1 $flowiter 3 $outputcelf"/flowstat_"$celf_mc".txt" $outputcelf"/spread.txt"
cp $outputcelf"/flowstat_"$celf_mc".txt_300" ../Results/freshRuns/processed/fig6e-7e-8e/celf.txt

echo "Running CELF++"
echo $execfilecelfpp" -propModel WC -mcruns "$celfpp_mc" -budget 200 -celfPlus 1 -probGraphFile "$pathtograph" -outdir "$outputcelfpp
$execfilecelfpp -propModel $model -mcruns $celfpp_mc -budget 200 -celfPlus 1 -probGraphFile $pathtograph -outdir $outputcelfpp
cp $outputcelfpp"/stats_CELF++_IC_"$celfpp_mc".txt" ../Results/freshRuns/processed/fig6e-7e-8e/celfpptime.txt

echo $execFlow $pathtograph $outputcelfpp"/seeds_CELF++_IC_"$celfpp_mc".txt" 200 1 $flowiter 3 $outputcelfpp"/flowstat_"$celfpp_mc".txt" $outputcelfpp"/spread.txt"
$execFlow $pathtograph $outputcelfpp"/seeds_CELF++_IC_"$celfpp_mc".txt" 200 1 $flowiter 3 $outputcelfpp"/flowstat_"$celfpp_mc".txt" $outputcelfpp"/spread.txt"
cp $outputcelfpp"/flowstat_"$celfpp_mc".txt_300" ../Results/freshRuns/processed/fig6e-7e-8e/celfpp.txt
