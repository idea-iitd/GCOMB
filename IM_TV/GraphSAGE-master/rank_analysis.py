...
budget_list =[10,20,50,100,150,200]#10,15,25,50,150,200]
num_k = None
x_axis=budget_list
y_axis= []
for num_k  in budget_list[0:]:
   ...
   # solution_file.readline()
    ...
    print("loaded")
    degree_of_nodes_weighted = G.out_degree(weight='weight')
    degree_of_nodes = G.out_degree()

  #  print("12960",degree_of_nodes_weighted[12960], degree_of_nodes[12960] )
    all_nodes = nx.nodes(G)

    sorted_ids_deg_wt = sorted(range(len(degree_of_nodes_weighted)),
                               key=lambda
                                   k:degree_of_nodes_weighted[k], reverse=True)

    print(sorted_ids_deg_wt[0:200])
   ...
   #     print(index, ids , degree_of_nodes[ids], degree_of_nodes[ids], "\n")
    ...
    for sol_id in optimal_nodes:
        print("rank of sol", sol_id,"is ", sorted_ids_deg_wt.index(sol_id), "with out_weight ", degree_of_nodes[sol_id],"degree ", degree_of_nodes_weighted[sol_id])
        ranks_list.append(sorted_ids_deg_wt.index(sol_id))
        if degree_of_nodes_weighted[sol_id] <= min_score:
            min_score = degree_of_nodes_weighted[sol_id]
...
#     interpol_file.write(str(x) +"\t" + str(y)+"\n")
#     interpol_dict[x] =y
#     print(x,y)
#
# interpol_file.close()
