...
class NodeMinibatchIterator(object):
    ...
    def __init__(self, G, id2idx,
        ...
        #					 0:int(len(G)) - int(0.1 * len(G) / 2)]
            ...
            print(self.all_nodes[0:5])
            print(self.label_map[63270])
            ...
            self.adj, self.deg, self.features, self.top_degree_nodes, time_sampling_feature, self.dict_map_couter_to_real_node,sampled_nbrs_for_rl=self.construct_adj_and_feat_for_top_degree_testing(
                budget, bud_mul_fac, sampling_freq)
    ...
    def construct_adj(self):
        ...
        #	print(nodeid, neighbors, self.G[nodeid])
            neighbors_weights=np.array([self.G[nodeid][neighbor]['weight']
    ...
    def construct_adj_and_feat_for_top_degree_testing(self, budget=None, bud_mul_fac=None, sampling_freq=None):
        ...
        total_top_degree_nodes=-1*int(bud_mul_fac+50)#int(-len(degree_of_nodes_all) * bud_mul_fac)  #
   #     total_top_degree_nodes=-1*int(bud_mul_fac+50)#int(-len(degree_of_nodes_all) * bud_mul_fac)  #
#CHANGED
        top_degree_nodes=set(np.argpartition(degree_of_nodes_all, total_top_degree_nodes)[total_top_degree_nodes:])
      #  top_degree_nodes=set(np.argpartition(degree_of_nodes_all, total_top_degree_nodes)[0:])

      #  top_degree_nodes=self.G.nodes()
